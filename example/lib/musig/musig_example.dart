...
void main() async {
  ...
  final input =
      TxInput(txId: '9bb72872594cb9b7a029f3daf0fe277dfbe5de3d52d4b55ba283cfa97b9310cc', txIndex: 1);
  ...
  final output = TxOutput(amount: rAmount, scriptPubKey: signer.toAddress().toScriptPubKey());
  final changeOutput = TxOutput(amount: change, scriptPubKey: address.toScriptPubKey());
  BtcTransaction transaction = BtcTransaction(inputs: [input], outputs: [output, changeOutput]);
...
BtcTransaction _spendWithKeyPath(
  ...
  final signer = Musig2Signer.generate([pk1, pk2, pk3], privateKeys: [key1, key2, key3]);
...
BtcTransaction _spendWithLeafA(
  ...
  final pk3 = key3.getPublic();
  final musigSigner =
      MuSig2.aggPublicKeys(keys: [pk1.publicKey.compressed, pk3.publicKey.compressed]);
  ...
  final digest = transaction
      ...
      .asImmutableBytes;
  final nonce1 =
      MuSig2.nonceGenerate(publicKey: pk1.toBytes(mode: PubKeyModes.compressed), msg: digest);
  final nonce3 =
      MuSig2.nonceGenerate(publicKey: pk3.toBytes(mode: PubKeyModes.compressed), msg: digest);
  ...
  final session = MuSig2Session(
      ...
      tweaks: [],
      msg: digest);
  final sig1 = MuSig2.sign(secnonce: nonce1.secnonce, sk: key1.toBytes(), session: session);
  final sig3 = MuSig2.sign(secnonce: nonce3.secnonce, sk: key3.toBytes(), session: session);
  final signature = MuSig2.partialSigAgg(signatures: [sig1, sig3], session: session);
...
BtcTransaction _spendWithLeafC(
  ...
  return transaction.copyWith(witnesses: [
    TxWitnessInput(stack: [sig2, sig1, leafC.script.toHex(), controlBlock.toHex()])
  ]);
}
...
Bip32Slip10Secp256k1 _deriveKey(
  ...
  Bip32Slip10Secp256k1 secp =
      Bip32Slip10Secp256k1.fromExtendedKey(key, Bip44Coins.bitcoinTestnet.conf.keyNetVer);
...
TaprootBranch _generateScriptTree() {
  ...
  final leaf2 = TaprootLeaf(script: Script(script: [pk3.toXOnlyHex(), BitcoinOpcode.opCheckSig]));
