...
void _test() {
  void test_(PsbtBuilder builder) {
    ...
    builder.addOutput(PsbtTransactionOutput(amount: amount1, address: recipient1()));
    builder.addOutput(PsbtTransactionOutput(amount: amount2, address: recipient2()));
    builder.addOutput(PsbtTransactionOutput(amount: amount3, address: recipient3()));

    /// sign input 1 signash signle
    builder.signInput(
        signer: (p0) {
          final signers =
              (expandUtxos[0]["privateKeys"] as List).map((e) => ECPrivate.fromHex(e)).toList();
          return PsbtSignerResponse(
              signers: signers.map((e) => PsbtDefaultSigner(e)).toList(),
              sighash: BitcoinOpCodeConst.sighashSingle,
              tapleafHash: BytesUtils.tryFromHexString(expandUtxos[0]["select_leaf"]));
        },
        index: 0);
    ...
    expect(
        () =>
            builder.updateOutput(0, PsbtTransactionOutput(amount: amount1, address: recipient2())),
        throwsA(isA<DartBitcoinPluginException>()
            .having((p0) => p0.message, 'error message', contains('Unable to modify output'))));
    ...
    final tx = signAll(builder: decode, start: 1, utxos: utxosVector2.expand((e) => e).toList());
    expect(tx.serialize(),
...
BtcTransaction signAll(
    {required PsbtBuilder builder, required int start, required List<Map<String, dynamic>> utxos}) {
  ...
  for (int i = start; i < utxos.length; i++) {
    ...
    bl.signInput(
      ...
      signer: (p0) {
        final signers =
            (utxos[p0.index]["privateKeys"] as List).map((e) => ECPrivate.fromHex(e)).toList();
        return PsbtSignerResponse(
            signers: signers.map((e) => PsbtDefaultSigner(e)).toList(),
            sighash: BitcoinOpCodeConst.sighashNone,
            tapleafHash: BytesUtils.tryFromHexString(utxos[p0.index]["select_leaf"]));
      },
    );
    bl.finalizeInput(
      i,
      onFinalizeInput: (params) {
        return PsbtFinalizeResponse(
            tapleafHash: BytesUtils.tryFromHexString(utxos[params.index]["select_leaf"]));
      },
    );
  }
...
void _test2() {
  void test_(PsbtBuilder builder) {
    ...
    sign(builder: builder, index: 0, utxos: utxos, sighash: BitcoinOpCodeConst.sighashSingle);
    ...
    sign(builder: builder, index: 1, utxos: utxos, sighash: BitcoinOpCodeConst.sighashNone);
    builder.addOutput(PsbtTransactionOutput(amount: amount2, address: recipient2()));
    builder.updateOutput(1, PsbtTransactionOutput(amount: amount2, address: recipient3()));
    ...
    sign(builder: builder, index: 2, utxos: utxos, sighash: BitcoinOpCodeConst.sighashAll);
    expect(
        () =>
            builder.updateOutput(0, PsbtTransactionOutput(amount: amount1, address: recipient2())),
        throwsA(isA<DartBitcoinPluginException>()
            .having((p0) => p0.message, 'error message', contains('Unable to modify output'))));
    expect(
        () => builder.addOutput(PsbtTransactionOutput(amount: amount1, address: recipient2())),
        throwsA(isA<DartBitcoinPluginException>()
            .having((p0) => p0.message, 'error message', contains('Unable to modify output'))));
...
BitcoinBaseAddress recipient2() {
  return P2shAddress.fromAddress(
      address: "2ND8PB9RrfCaAcjfjP1Y6nAgFd9zWHYX4DN", network: BitcoinNetwork.testnet);
}

BitcoinBaseAddress recipient3() {
  return P2shAddress.fromAddress(
      address: "2MuTei1oXmMKbmnDBCfKCtrbWDdbnriYZXm", network: BitcoinNetwork.testnet);
}

BitcoinBaseAddress recipient4() {
  return P2pkhAddress.fromAddress(
      address: "modADgm9UHadfWLQVKeHapbHjETkwwY6zs", network: BitcoinNetwork.testnet);
}
