...
void _test(PsbtBuilder builder) {
  ...
  final totalAmoumt = psbtUtxos.fold<BigInt>(BigInt.zero, (p, c) => p + c.utxo.value);
  ...
  builder.signInput(
      signer: (params) {
        ...
        expect(utxo[0]["address"], params.address.toAddress(BitcoinNetwork.testnet));
        final signers = (utxo[0]["privateKeys"] as List).map((e) => ECPrivate.fromHex(e)).toList();
  ...
  builder.signInput(
      signer: (params) {
        ...
        final utxo = utxos[1];
        expect(utxo.length, 2);
        expect(utxo[1]["address"], params.address.toAddress(BitcoinNetwork.testnet));
        final signers = (utxo[1]["privateKeys"] as List).map((e) => ECPrivate.fromHex(e)).toList();
  ...
  builder.signAllInput(
    (p0) {
      if (p0.inputData.partialSigs != null || p0.inputData.taprootScriptSpendSignature != null) {
        return null;
      }
      final signers =
          (expandUtxos[p0.index]["privateKeys"] as List).map((e) => ECPrivate.fromHex(e)).toList();
      return PsbtSignerResponse(
          signers: signers.map((e) => PsbtDefaultSigner(e)).toList(),
          sighash: BitcoinOpCodeConst.sighashAll,
          tapleafHash: BytesUtils.tryFromHexString(expandUtxos[p0.index]["select_leaf"]));
    },
  );
  expect(
      ...
      throwsA(isA<DartBitcoinPluginException>().having(
          (p0) => p0.message, 'error message', contains('Missing input non-witness UTXOs'))));
  final key14 = PsbtDefaultSigner(
      ECPrivate.fromHex("efd4d62709543c1bae8a22b44640856b48398b7d06f57958760a8695b9f5f171"));
  final newBuilder = PsbtBuilder.fromBase64(builder.toBase64());
  newBuilder.signInput(signer: (params) => PsbtSignerResponse(signers: [key14]), index: 14);
    ...
    onFinalizeInput: (params) {
      if (params.inputData.taprootLeafScript != null &&
        ...
        return PsbtFinalizeResponse(
            tapleafHash: BytesUtils.tryFromHexString(expandUtxos[params.index]["select_leaf"]));
      }
      return null;
    },
  );
