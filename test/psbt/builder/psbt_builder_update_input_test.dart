  ...
  void test_(PsbtBuilder builder, PsbtVersion version) {
    ...
    expect(
        ...
        throwsA(isA<DartBitcoinPluginException>()
            .having((p0) => p0.message, 'error message', contains('Duplicate input detected'))));
    ...
    expect(
        () =>
            builder.updateOutput(0, PsbtTransactionOutput(amount: amount2, address: recipient2())),
        throwsA(isA<DartBitcoinPluginException>()
    ...
    builder.updateOutput(1, PsbtTransactionOutput(amount: amount3, address: recipient3()));
    ...
    builder = signFinalize(
        builder: builder, index: 2, utxos: expandUtxos, sighash: BitcoinOpCodeConst.sighashAll);
    expect(
        () => builder.addOutput(PsbtTransactionOutput(amount: amount2, address: recipient4())),
        throwsA(isA<DartBitcoinPluginException>()
            .having((p0) => p0.message, 'error message', contains('Unable to modify output'))));
    expect(
        () => builder.addUtxo(psbtUtxos[3]),
        throwsA(isA<DartBitcoinPluginException>()
            .having((p0) => p0.message, 'error message', contains('Unable to modify input'))));
...
void _test1() {
  test("description", () {
    ...
    builder = signFinalize(
        builder: builder, index: 0, utxos: expandUtxos, sighash: BitcoinOpCodeConst.sighashAll);
    ...
    expect(
        () => builder.psbtInput(0),
        throwsA(isA<DartBitcoinPluginException>()
            .having((p0) => p0.message, 'error message', contains('Invalid input index'))));
    builder.addUtxo(psbtUtxos[0]);
    final amount2 = BtcUtils.toSatoshi('0.0011');
    final amount3 = BtcUtils.toSatoshi('0.0012');
    builder.addOutput(PsbtTransactionOutput(amount: amount2, address: recipient1()));
    builder.addOutput(PsbtTransactionOutput(amount: amount3, address: recipient2()));
    builder = signFinalize(
        builder: builder,
        index: 0,
        utxos: expandUtxos,
    ...
    builder = signFinalize(
        builder: builder, index: 2, utxos: expandUtxos, sighash: BitcoinOpCodeConst.sighashAll);
    final tx = builder.finalizeAll();
    expect(tx.txId(), "f9a4f122347356516a40f49ab5b14d71ae4f88ec82b285dff5835f9ec9dc3e2b");
  });
}

void _expectInput({required PsbtBuilder builder, required PsbtUtxo utxo, required int index}) {
